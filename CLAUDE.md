...
Current subjects: `javascript`, `react`, `node`, `typescript`, `aws`, `graphql`, `springboot`, `java`, `python`, `docker`, `cloud-project`, `kubernetes`.
...
| Cloud Project | 70 | Interview prep grounded in the **`training-docker`** companion repo (a real AppSync→ECS→DynamoDB stack). Includes a layer of **easy "what is / why" foundations** (GraphQL, AppSync, Docker, ECS, ALB, Terraform/IaC, DynamoDB). **All 70 cards have deep dives** (`data/deepdives/cloud-project.json`) — explanation + fenced example where useful + links to official docs (graphql.org, AWS, Docker, HashiCorp, GitHub, MCP/OWASP). Topics map 1:1 to that project's code: GraphQL (basics, REST vs GraphQL, over-fetching, schema/query/resolver), Architecture (request flow), AppSync (what/benefits, heterogeneous schema, APPSYNC_JS resolvers, GetItem vs Scan, API-key→IAM auth, pagination), DynamoDB (what is it, key schema, Query vs Scan, on-demand, GSI access patterns), Express Client (GraphQL client over `fetch`, error layers, XSS/`esc()`, health checks), Docker (what/image vs container/Dockerfile, multi-stage, `npm ci`, layer caching, non-root, arch mismatch), ECS & ALB (what/why orchestrate, Fargate vs EC2, task vs execution role, secret injection, SG chain, rolling deploys), Terraform (what/why IaC, `data` vs `resource`, secret wiring, sensitive outputs, state, `create_before_destroy`), Claude CI (PR trigger, `fetch-depth: 0`, secrets, workflow permissions, MCP allow-list, review scoping), Security & Cost (secrets inventory, cost model, production-readiness, least privilege). Reveal + MCQ + multiple-response. Companion study doc: `training-docker/INTERVIEW.md` |
...
> Status: the **dev** stack is deployed and the dev stage wired + seeded; prod is not. Frontend env is set in Vercel (login live). `drill-cards-dev` holds all twelve banks (1177 cards incl. kubernetes's 56, deep dives merged on each item); re-run `seed-cards.mjs` after editing `app/data/*.json`.

...
Current subjects: `javascript`, `react`, `node`, `typescript`, `aws`, `graphql`, `springboot`, `java`, `python`, `docker`, `kubernetes`, `regex`.
...
> Status: the **dev** stack is deployed and the dev stage wired + seeded; prod is not. Frontend env is set in Vercel (login live). `drill-cards-dev` holds all twelve banks (1150 cards, deep dives merged on each item); re-run `seed-cards.mjs` after editing `app/data/*.json`.

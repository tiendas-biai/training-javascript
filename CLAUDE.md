...
2. **`app/`** — A Vite + React + TypeScript spaced-repetition drill app (Dev Drill) with separate question banks for JavaScript, TypeScript, React, Node.js, GraphQL, and AWS SAA. Anonymous users work against `localStorage`; signed-in users get **Auth0 login + per-user cloud progress**.
...
graphql: {
  id: 'graphql', label: 'GraphQL', icon: '◈', color: '#e10098',
  storageKey: 'srs:graphql',
  loadData: () => load(import('../../data/graphql.json')),
},
```
...
Current subjects: `javascript`, `react`, `react-concepts`, `node`, `typescript`, `aws`, `graphql`, `springboot`, `java`, `python`, `docker`, `kubernetes`, `regex`, `karat`, `cnsc-docente`.
...
| React Concepts | 22 | Interview-style concept sweep, one reveal card per concept ("What is X?" + concise definition + short example) — no MCQ/MR. **All 22 have deep dives** (`data/deepdives/react-concepts.json`). Topics: Core Concepts (components, JSX, state, lifecycle, hooks, composition, React DOM, Virtual DOM, forms, context), Patterns & Quality (performance, code reuse, security, typechecking), Redux (Redux Toolkit flavor: core, React Redux, async thunks), Ecosystem (routing/React Router, testing/Jest+RTL, building/Vite, SSR, SSG) |
...
| Kubernetes | 56 | Kubernetes basics + AWS integrations, scoped for learning alongside the `training-docker` work. **All 56 cards have deep dives** (`data/deepdives/kubernetes.json`) with official-doc links (kubernetes.io, AWS EKS, Terraform registry). Topics: Fundamentals (what is k8s, orchestration, declarative model, manifests, vs Docker), Architecture (control plane vs nodes, etcd, kubelet/kube-proxy, API server), Workloads (Pods, Deployments, ReplicaSets, probes, requests/limits, HPA, rollouts, controllers), Networking (Services + types, selectors, Ingress, pod networking/CNI), Config & Namespaces (ConfigMaps, Secrets + base64 caveat, namespaces, labels), **kubectl** (get/describe/logs/exec/apply/scale/rollout/contexts terminal commands), EKS (what/shared responsibility, node groups vs Fargate, update-kubeconfig, IAM↔RBAC, ALB controller), AWS Integrations (IRSA, pod→DynamoDB, pod→Lambda, EKS vs Lambda, ACK), Terraform (provisioning EKS via the module, kubernetes/helm providers, IRSA via TF). Reveal + MCQ + multiple-response |
| Regex (JS) | 43 | JavaScript regex from scratch (easy→medium), centered on the string/RegExp methods. **All 43 cards have deep dives** (`data/deepdives/regex.json`) with runnable `js` examples (verified via `verify-node-examples.mjs`) and MDN links. Topics: Regex Basics (literals vs constructor, escaping), Flags (g/i/m, dotAll vs multiline), Character Classes (`\d \w \s`, the dot, ranges, negation), Anchors (`^ $`, `\b`), Quantifiers (`* + ?`, `{n}`, greedy vs lazy), Groups & Alternation (capturing, `(?:)`, named groups, `|`), String & RegExp Methods (test, match, matchAll, replace, replaceAll, search, split, exec + lastIndex), Putting It Together (validation, extraction, method choice). Reveal + MCQ + multiple-response |
| Karat Prep | 72 | Prep for the Karat (karat.com) JS/Node technical assessment, built from first-hand reports in `features/karat/karat_assesment.xlsx` + web research. **All 72 cards have deep dives** (`data/deepdives/karat.json`, runnable `js` examples verified via `verify-node-examples.mjs karat`). Topics: Code Review (18 — Block 1 exhibits: SQL injection, hardcoded secrets, eval/RCE, sync fs, missing await, check-then-act races, float money, mutable map keys, refactoring drills; the 4-question method), Live Exhibits (10 — full-block drill exhibits mirroring a real session: hand-rolled `Array.prototype.reverse`, mysql2 query→loop→reverse, package.json with the fake `echo "no tests"` test script + lockfile, substring-vs-slice, N+1 query loop, IN-clause building, CSV parser, PAN logging, one capstone combining them; ids `karat-le-*`), Coding Patterns (22 — 2 cards per reported exercise in `features/karat/exercises/01…11`), More Problems (8 — intervals/sweep line, sliding window, canonical-key grouping, check-then-commit), Interview Format (6 — structure, ≥2-of-3 pass bar, narration, redo policy), Domain Knowledge (8 — cookies vs localStorage, REST vs GraphQL, event loop, HTTP caching, SQL vs NoSQL, idempotency, N+1, pagination). Companion practice material in `features/karat/` (code-review snippets 01–07 + REVIEW.md model answers, CommonJS exercises 12–15 with Jest tests, PLANNING.md + STUDY_GUIDE.md) |
| Concurso Docente CNSC | 90 | **The only Spanish-language bank** (UI stays English). Prep for the Colombian CNSC teacher-selection exam (Docente de aula, Lengua Castellana), built from Jira EB-736 + its attachment `concurso_docente_cnsc_2026.md`, with norms verified against the official texts (Función Pública Gestor Normativo, RAE DPD, MEN). Mirrors the exam format: 81 MCQ with exactly 4 options + 9 multiple-response ("(Seleccione DOS/TRES)"), mostly classroom-case questions. **All 90 cards have deep dives** (`data/deepdives/cnsc-docente.json`, Spanish labels `**El problema:**` / `**Por qué es correcta:**` / `**Por qué fallan los distractores:**` / `**Clave para el examen:**`; prose-only examples — no code, so no verifier; `RichText` doesn't render markdown tables/blockquotes, so content uses plain lines and `•` bullets). Topics = the 5 fronts of the eliminatory test + the psychometric test: Lectura Crítica (14), Razonamiento Cuantitativo (12), Competencias Pedagógicas (16 — evaluación/Decreto 1290, Ausubel/Vygotsky/Bloom, inclusión/Decreto 1421/PIAR/DUA, PEI/currículo), Lengua Castellana (20 — ortografía RAE 2010, gramática/sintaxis, semántica, tipologías, Jakobson, literatura colombiana, Estándares del Lenguaje, escritura como proceso), Normativa Educativa (12 — Ley 115, gobierno escolar, Decreto 1278 escalafón/periodo de prueba/evaluación, Decreto 1075, Ley 1620/Decreto 1965), Competencias Comportamentales (6 — Guía 31), Juicio Situacional (6), Estructura del Concurso (4 — figures flagged as provisional until the official Acuerdo/Anexo Técnico) |
...
- **Authoring** — run the per-subject prompt in `documents/prompts/` against a card, then merge the result via `app/scripts/dd-batch.mjs` and verify the example compiles: `app/scripts/verify-deepdive-examples.mjs <subject>` (tsx type-check, for React/TS), `app/scripts/verify-node-examples.mjs <subject>` (`node --check` syntax pass, for JS/Node `js` snippets), `app/scripts/verify-graphql-examples.mjs graphql` (routes ` ```graphql ` blocks through `parse()` from the `graphql` package and ` ```js ` blocks through `node --check`), `app/scripts/verify-java-examples.mjs java` (wraps each ` ```java ` fragment into a compilable class — imports hoisted + a standard set injected, top-level types made `static` nested, file-scope methods made `static`, loose statements into `main` — then runs `javac`; needs a JDK 17+ on PATH, e.g. `PATH="$(brew --prefix openjdk@21)/bin:$PATH"`, and SKIPs cleanly if absent), `app/scripts/verify-python-examples.mjs python` (runs each ` ```python ` snippet through CPython's `python3 -m py_compile` — syntax-only parse, no execution/import resolution; SKIPs cleanly if no `python3`), `app/scripts/verify-docker-examples.mjs docker` (routes by fence: ` ```yaml ` parsed with the `yaml` package, ` ```bash ` via `bash -n`, ` ```dockerfile ` via `hadolint` if present else an instruction lint; `json`/other fences skipped), or `app/scripts/verify-aws-examples.mjs aws` (` ```json ` via `JSON.parse`, ` ```yaml ` via the `yaml` package, ` ```bash ` via `bash -n`; other fences skipped). **Author in small per-topic batches, never one giant pass** (a one-shot agent attempt faked 187/200 cards). Write the explanation's bold section labels ending in a **colon** (`**The problem:**`) — the `DeepDive` spacing regex only fires on `:**`. **Status: all fifteen banks are 100% done** (React, React Concepts, Node.js, TypeScript, JavaScript, GraphQL, Spring Boot, Java, Python, Docker, AWS, Kubernetes, Regex, Karat, Concurso Docente CNSC). Full runbook + per-subject/per-topic content status + the next-batch recipe in **`documents/DEEP_DIVES.md`**.
...
- **Fenced code blocks** — ` ```js\n...\n``` ` are rendered as highlighted `<pre>` blocks using **Prism.js** (One Dark token colors; the only `dangerouslySetInnerHTML` in the app — Prism output over our own JSON). `js`/`ts`/`jsx`/`tsx`/`graphql`/`java`/`python`/`bash`/`yaml`/`docker` (Dockerfile) grammars are loaded; other languages fall back to the JS grammar. Each block has a hover **Copy** button.

...
| Karat Prep | 62 | Prep for the Karat (karat.com) JS/Node technical assessment, built from first-hand reports in `features/karat/karat_assesment.xlsx` + web research. **All 62 cards have deep dives** (`data/deepdives/karat.json`, runnable `js` examples verified via `verify-node-examples.mjs karat`). Topics: Code Review (18 — Block 1 exhibits: SQL injection, hardcoded secrets, eval/RCE, sync fs, missing await, check-then-act races, float money, mutable map keys, refactoring drills; the 4-question method), Coding Patterns (22 — 2 cards per reported exercise in `features/karat/exercises/01…11`), More Problems (8 — intervals/sweep line, sliding window, canonical-key grouping, check-then-commit), Interview Format (6 — structure, ≥2-of-3 pass bar, narration, redo policy), Domain Knowledge (8 — cookies vs localStorage, REST vs GraphQL, event loop, HTTP caching, SQL vs NoSQL, idempotency, N+1, pagination). Companion practice material in `features/karat/` (code-review snippets + REVIEW.md model answers, CommonJS exercises 12–14 with Jest tests, PLANNING.md + STUDY_GUIDE.md) |
